...
in this exact order to initialize the Viz COMP''')
	...
	def InitViz(self):
		op('replicator_ctrl').par.recreateall.pulse()
		op('replicator_viz').par.recreateall.pulse()

		
	def InitConnections(self):
		op('viz1').inputConnectors[0].connect(op('_ctrl1').outputConnectors[0])
		op('viz2').inputConnectors[0].connect(op('_ctrl2').outputConnectors[0])
		op('viz3').inputConnectors[0].connect(op('_ctrl3').outputConnectors[0])
		op('viz4').inputConnectors[0].connect(op('_ctrl4').outputConnectors[0])
		op('viz5').inputConnectors[0].connect(op('_ctrl5').outputConnectors[0])
		op('viz6').inputConnectors[0].connect(op('_ctrl6').outputConnectors[0])
